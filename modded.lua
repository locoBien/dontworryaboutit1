        ...
        function window:Watermark(info)
            ...
            utility:Connection(rs.RenderStepped, function(FPS)
                ...
                if (tick() - Tick) > 0.15 then
                    watermark_title.Text = string.format("[Private]  [Ping : %s ]  [Fps : %u] [UID : %a] [ACC : %i]", library.shared.ping, library.shared.fps, game.Players.LocalPlayer.UserId, game.Players.LocalPlayer.Name)
                    window.watermark:UpdateSize()
                    --
                    Tick = tick()
                end
